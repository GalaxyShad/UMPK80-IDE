...
export function SettingsContext({children}: { children: React.ReactNode }) {
  ...
  const [translatorCommand, setTranslatorCommand] = useState<string>(useTranslatorStore((state) => state.translateCommand))
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4 py-4">
          <div className="flex gap-2 items-center">
            <Checkbox checked={isUsingCustomTranslatorPath}
          ...
          {isUsingCustomTranslatorPath &&
              <div className="flex gap-2">
                  <Input value={translatorCommand} onChange={(e) => setTranslatorCommand(e.target.value)}
                         placeholder="Путь к исполняемому файлу"/>
                  <Button onClick={selectFromDialog} size="icon"><FolderOpen/></Button>
              </div>}
          <Input className={isOutputError ? 'border-red-500' : ''} readOnly value={translatorCommand + ' -v'}/>
          <div className={cn("border rounded px-2 py-2", isOutputError ? 'border-red-500' : '')}>
            {output}
          </div>
        </div>
        <DialogFooter>
          <Button onClick={trySetTranslatorCommand} type="submit">Сохранить</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
